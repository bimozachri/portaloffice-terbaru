...
export default function SatsetPage() {
  ...
  // Link GAS Satset (Silakan isi dengan link deploy milikmu nanti)
  const urlSatset: string = "https://script.google.com/macros/s/AKfycbx7sNOIictCtxihivdjGCYE0qMjuWCa9RVNDHWzsmUJfw6mtkHhRpRoWzjgYoD7NTAa/exec"; 

  // Fungsi pengecekan password lapis pertama
  const handleLogin = (e: React.FormEvent) => {
    e.preventDefault();
    // Silakan ganti "satset2026" dengan password yang kamu inginkan
    ...
    <div className="h-[100dvh] flex flex-col bg-background overflow-hidden">
        ...
        <header className="bg-background/95 backdrop-blur-md border-b border-border">
          <div className="container mx-auto px-4 h-16 flex items-center justify-between">
            <Link href="/" className="flex items-center gap-3">
              <Image src="/images/forms/ab-logo.png" alt="Angsae Baru Logo" width={40} height={40} className="object-contain" />
      ...
      <main className="flex-1 w-full relative bg-slate-50/50 dark:bg-transparent">
        {!isAuthenticated ? (
          ...
          <div className="absolute inset-0 overflow-y-auto flex items-center justify-center p-4">
            <div className="w-full max-w-sm bg-card border border-border rounded-xl shadow-sm p-6 text-center">
              ...
              <p className="text-sm text-muted-foreground mb-6">
                Aplikasi Satset sedang dalam tahap pengembangan. Silakan masukkan password untuk mengakses pratinjau.
              </p>
          ...
          /* Iframe Satset muncul jika password benar */
          urlSatset !== "" ? (
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              <span className="text-muted-foreground font-medium bg-background px-4 py-2 rounded-lg border border-border shadow-sm">
                Link sistem Satset belum dikonfigurasi.
              </span>
            </div>
          )
